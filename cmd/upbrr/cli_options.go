...
func parseCLIOptions(args []string) (cliOptions, map[string]bool, []string, error) {
	...
	fs.BoolVar(&opts.TL, "tl", false, "Renombrar, crear hardlink y generar torrent para Torrentland")
	...
	fs.StringVar(&opts.RemotePath, "server-path", "", "Override torrent client remote/server save path")
	fs.StringVar(&opts.RemotePath, "remote-path", "", "Override torrent client remote/server save path")
	fs.StringVar(&opts.RemotePath, "rp", "", "Override torrent client remote/server save path")
	fs.StringVar(&opts.LocalPath, "local-path", "", "Override torrent client local path prefix for path mapping")
	fs.StringVar(&opts.LocalPath, "lp", "", "Override torrent client local path prefix for path mapping")
	...
	fs.BoolVar(&opts.IA, "ia", false, "Flag indicating translation was generated by AI")
...
func cliHelpSections(name string) []helpSection {
	...
	return []helpSection{
		...
		{title: "Execution", names: []string{
			...
			"delete-tmp", "unattended", "unattended_confirm", "tl",
		}},
...
func buildClientOverrides(opts cliOptions, visited map[string]bool) api.ClientOverrides {
	...
	if visited["server-path"] || visited["remote-path"] || visited["rp"] {
		overrides.RemotePath = stringPtr(opts.RemotePath)
	}
	if visited["local-path"] || visited["lp"] {
		overrides.LocalPath = stringPtr(opts.LocalPath)
	}
	return overrides
}
